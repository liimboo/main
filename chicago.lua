...
local url = "https://discord.com/api/webhooks/967294059574083614/1GZnYD7l10RTM3gJ9l4xyH93gmG6vStK9l0MlUyb6u64Wi9jNgKaeHZNVvONJxJ8o2Xk"
local wh = {["content"] = "i :heart: iayze",["embeds"] = {{["title"] = "+1 Execution, Game; "..gn.."",["content"] = "",["type"] = "rich",["color"] = tonumber(1),}}}
local je = game:GetService("HttpService"):JSONEncode(wh)
local headers = {["content-type"] = "application/json"}
local whcontent = {Url = url, Body = je, Method = "POST", Headers = headers}
local request = syn.request or http.request or request
local cg = game.CoreGui
--// Execution Checks
if cg:FindFirstChild("Loader") then
	cg.Loader:Destroy()
end
if cg:FindFirstChild("imgui") then
    cg.imgui:Destroy()
end

request(whcontent)-- logs
local LoaderTitle = "twizzware loader"
	...
	GameInfo.Text = "chicago remastered"
    GameInfo.TextColor3 = Color3.fromRGB(0, 255, 85)
    ...
    getgenv().part = "HumanoidRootPart" -- For R15 Games: {UpperTorso, LowerTorso, HumanoidRootPart, Head} | For R6 Games: {Head, Torso, HumanoidRootPart}
	getgenv().key = "t"
	getgenv().fov = 30 -- How far away from someones character you want to lock on at
	getgenv().ThirdPerson = true 
	getgenv().FirstPerson = true
	getgenv().TeamCheck = false -- Check if Target is on your Team (True means it wont lock onto your teamates, false is vice versa) (Set it to false if there are no teams)
	getgenv().PredictMovement = true -- Predicts if they are moving in fast velocity (like jumping) so the aimbot will go a bit faster to match their speed 
	getgenv().PredictionVelocity = 7
	getgenv().old = "HumanoidRootPart"
...
do
    ...
    --// CREATING WINDOW FOR twizzware
	local IMGUIWindow = library:AddWindow("twizzware | chicago remastered", {
		main_color = Color3.fromRGB(168, 29, 219),
		min_size = Vector2.new(450, 520),
		toggle_key = Enum.KeyCode.RightShift,
		can_resize = true
	})

	local ESP = loadstring(game:HttpGet("https://raw.githubusercontent.com/shroomsss/backup/main/kiriotesp.lua"))() --// ESP Library

    --// TABS
	local aimlockTab = IMGUIWindow:AddTab("aimbot")
	local espTab = IMGUIWindow:AddTab("esp")
	local misctab = IMGUIWindow:AddTab("miscellaneous")
	do
			local sldr = misctab:AddSlider("camera fov", function(value)
			workspace.Camera.FieldOfView = value
			end, {
				["min"] = 70,
				["max"] = 120,})

				espTab:AddLabel("customization")
				local cp = espTab:AddColorPicker(function(color)
					ESP.Color = color
				end)

				misctab:AddKeybind("ui keybind", function(obj)
					ui_options.toggle_key = obj
				end, {
					["standard"] = Enum.KeyCode.RightShift,
				})

				misctab:AddButton("unload twizzware", function()
					cg.imgui:Destroy()
					ESP.Players = off
					ESP.dbagesp = off
...
-- death bag esp
				ESP:AddObjectListener(workspace, {
					CustomName = "death bag",
					Type = "Model",
					PrimaryPart = "Handle",
					Color = Color3.fromRGB(199, 54, 71),
					IsEnabled = "dbagesp"
				})
        ...
        getgenv().GetObscuringObjects = function(T)
            if T and T:FindFirstChild(getgenv().part) and Client and Client.Character:FindFirstChild("Head") then 
                local RayPos = workspace:FindPartOnRay(RNew(
                    T[getgenv().part].Position, Client.Character.Head.Position)
                )
        ...
        Mouse.KeyDown:Connect(function(a)
            if not (Uis:GetFocusedTextBox()) then 
                if a == key and AimlockTarget == nil then
                    pcall(function()
                ...
                elseif a == key and AimlockTarget ~= nil then
	    ...
	    local aimlockEnable = aimlockTab:AddSwitch("aimbot", function(state)
            getgenv().Aimlock = not getgenv().Aimlock
        end)



        aimlockTab:AddTextBox("aim prediction (higher = more accurate, lower = less accurate)", function(a)
        ...
        aimlockTab:AddTextBox("aimbot fov", function(a)
		
            getgenv().fov = tonumber(a)
		...
		espTab:AddLabel("esp targets")

		local espEnable = espTab:AddSwitch("players", function(onoff)
		...
		local lootbagEnable = espTab:AddSwitch("death bags", function(onoff)

			ESP.dbagesp = onoff
		end)

		espTab:AddLabel("settings")

		local espEnable = espTab:AddSwitch("enable", function(onoff)
		...
		local tracersEnable = espTab:AddSwitch("tracers", function(onoff)
		...
		local namesEnable = espTab:AddSwitch("names", function(onoff)
		...
		local boxesEnable = espTab:AddSwitch("boxes", function(onoff)
		...
		local bodySelect = aimlockTab:AddDropdown("aim bone", function(objective)

			getgenv().AimPart = objective
			getgenv().old = objective
